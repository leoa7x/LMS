...
export default function TeacherEvaluationsPage() {
  ...
  return (
    <PortalShell
      eyebrow="Docente"
      title="Evaluaciones y reintentos"
      description="Quizzes visibles segun alcance y autorizacion de reintentos por parte del docente."
    >
      <RoleGuard roles={["TEACHER", "ADMIN"]}>
        ...
        <section className="grid gap-6 xl:grid-cols-[1fr_1fr]">
          ...
          <DataPanel title="Autorizar reintento">
            <form className="grid gap-4" onSubmit={createRetakeGrant}>
              ...
              <textarea className="min-h-28 rounded-2xl border border-slate-300 px-4 py-3 text-sm" placeholder="Motivo" value={grantForm.reason} onChange={(event)=>setGrantForm((prev)=>({...prev,reason:event.target.value}))}/>
        ...
        <section className="mt-6">
          <DataPanel title="Reintentos activos">
            <SimpleTable
              columns={[
              ...
              rows={retakeGrants}
              emptyLabel="No hay reintentos visibles."
            />
          </DataPanel>
        </section>
      </RoleGuard>
