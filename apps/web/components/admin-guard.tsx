  ...
  if (!isReady) {
    return (
      <div className="rounded-3xl border border-slate-200 bg-white p-8 text-sm text-slate-600 shadow-sm">
        Cargando informacion de acceso...
      </div>
    );
  }
    ...
    return (
      <div className="rounded-3xl border border-amber-200 bg-amber-50 p-8 shadow-sm">
        ...
        <p className="mt-2 text-sm leading-6 text-amber-800">
          Debes iniciar sesion para acceder a la administracion de la plataforma.
        </p>
    ...
    <>
      <div className="mb-4 flex items-center justify-between rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm shadow-sm">
        <p className="text-slate-600">
          Usuario activo: <span className="font-medium text-slate-900">{user?.email}</span>
        </p>
        <button
